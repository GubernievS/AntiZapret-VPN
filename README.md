...
**Внимание!** Для правильной работы AntiZapret VPN нужно:
1. [Отключить безопасный или сторонний DNS в браузере](https://www.google.ru/search?q=отключить+DNS+в+браузере)
2. Если в браузере есть проблемы с загрузкой или отображением страниц и картинок, то в браузере открываем: [chrome://flags/#local-network-access-check](chrome://flags/#local-network-access-check) и отключаем опцию Local Network Access Checks
...
Внимание! Серверы Аезы могут блокироваться в вашем регионе, в Москве и МО они доступны и работают
