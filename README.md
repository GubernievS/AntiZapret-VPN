...
По умолчанию используется протокол шифрования AES-128-GCM, если ваше устройство не поддерживает аппаратное шифрование AES-NI, например роутер, то рекомендуется попробовать в файле подключения (\*.ovpn) заменить AES-128-GCM на CHACHA20-POLY1305
