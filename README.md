...
Через AntiZapret VPN работают только (список автоматически обновляется раз в сутки):
- Заблокированные сайты из единого реестра Роскомнадзора
...
```
3. Дождаться перезагрузки сервера и скопировать файлы подключений (*.ovpn или *.conf) с сервера из папки /root, после загрузки будет обновляться список заблокированных сайтов, нужно подождать несколько минут
