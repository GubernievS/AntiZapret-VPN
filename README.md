...
- Сканирование портов: более 3 попыток подключения по разным портам и протоколам в первую минуту
- Большое число попыток подключения: более 1000 подключений в первую минуту
Если до блокировки остановить сканирование или попытки подключения более чем на 20 секунд лимиты попыток сбрасываются
После блокировки любые попытки подключения продлевают блокировку до 10 минут
ICMP отключен, кроме ICMP Fragmentation Needed и ICMPv6 Packet Too Big
> Список заблокированных IPv4 и IPv6-адресов
```sh
ipset list antizapret-blocklist && ipset list antizapret-blocklist6
```
> Список отслеживаемых подключений по IPv4 и IPv6 за последние 20 секунд
```sh
ipset list antizapret-watchlist && ipset list antizapret-watchlist6
```
