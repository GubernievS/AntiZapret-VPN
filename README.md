...
DNS антизапрета = 10.29.0.1 (172.29.0.1)\
Клиенты AntiZapret VPN = 10.29.0.0/16 (172.29.0.0/16)\
Клиенты обычного VPN = 10.28.0.0/16 (172.28.0.0/16)\
Подменные IP = 10.30.0.0/15 (172.30.0.0/15)\
В скобках - альтернативные IP-адреса
...
IPv4- или IPv6-адрес блокируется на 10 минут при нарушении следующих правил:
- Сканирование портов: более 10 попыток подключения с одного IP-адреса или суммарно со всех IP-адресов подсети /24 по разным портам и протоколам
- DDoS-атака: более 10 000 новых подключений с одного IP-адреса по разным портам и протоколам
Если в течение одной минуты с IP-адреса или со всех IP-адресов подсети /24 не было попыток подключения, лимиты сбрасываются\
При попытке подключения с заблокированного IP-адреса блокировка продлевается до 10 минут\
Если IPv4- или IPv6-адрес добавлен в список исключений, блокировка на него не распространяется\
